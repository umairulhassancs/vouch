...
function eyebrow(slide, text, x, y, opts = {}) {
  slide.addText(text.toUpperCase(), {
    x, y, w: opts.w || 7, h: 0.32, fontFace: FONT, fontSize: 12.5, bold: true,
    color: opts.color || C.purple, charSpacing: 2.4,
  });
}
function slideTitle(slide, text, x, y, opts = {}) {
  slide.addText(text, {
    x, y, w: opts.w || 8.6, h: opts.h || 0.85, fontFace: FONT_HEAD, fontSize: opts.size || 30,
...
// ================= SLIDE 1 — TITLE =================
{
  ...
  s.addText("Principles of Marketing — Project Presentation", {
    x: 0, y: 3.35, w: PGW, h: 0.4, align: "center", fontFace: FONT, fontSize: 13,
  ...
  s.addText("Smart Lost & Found Ecosystem", {
    x: 0, y: 4.7, w: PGW, h: 0.45, align: "center", fontFace: FONT, fontSize: 17,
    color: C.textOnNavy,
  });

  s.addText('"Lost, But Never Gone."', {
    x: 0, y: 5.15, w: PGW, h: 0.4, align: "center", fontFace: FONT_HEAD, fontSize: 15,
  ...
  names.forEach((n, i) => {
    ...
    s.addText(n[0], { x: x + 0.12, y: y + 0.09, w: cw - 0.24, h: 0.35, fontFace: FONT, fontSize: 12.5, bold: true, color: C.white });
    s.addText("Roll No. " + n[1], { x: x + 0.12, y: y + 0.44, w: cw - 0.24, h: 0.3, fontFace: FONT, fontSize: 10, color: C.lavender });
  });
}
...
// ================= SLIDE 2 — AGENDA =================
{
  ...
  const items = [
    ["search", "Research & Problem", "Methodology, problem identification & market opportunity"],
    ["bulb", "The Solution", "Vouch ecosystem, product line & platform"],
    ["users", "Market Analysis", "Target segments, personas & SWOT"],
    ["compass", "Marketing Strategy", "STP, value proposition, positioning & branding"],
    ["dollar", "The Marketing Mix", "Pricing, promotion & distribution (place)"],
    ["flag", "Wrap-Up", "Competitive view, challenges, roadmap & takeaways"],
  ];
  ...
  items.forEach((it, i) => {
    ...
    s.addText(it[1], { x: x + 0.28, y: y + 1.02, w: cw - 0.55, h: 0.4, fontFace: FONT, fontSize: 15, bold: true, color: C.white });
    s.addText(it[2], { x: x + 0.28, y: y + 1.4, w: cw - 0.55, h: 0.55, fontFace: FONT, fontSize: 10.5, color: C.textOnNavyMuted, lineSpacingMultiple: 1.15 });
  });
...
// ================= SLIDE 3 — EXECUTIVE SUMMARY =================
{
  ...
  ], { x: 0.7, y: 1.75, w: 6.9, h: 1.7, fontFace: FONT, fontSize: 13.5, color: C.textDark, lineSpacingMultiple: 1.28 });
  ...
  ], { x: 0.7, y: 3.4, w: 6.9, h: 1.3, fontFace: FONT, fontSize: 13.5, color: C.textDark, lineSpacingMultiple: 1.28 });

  card(s, 0.7, 4.85, 6.9, 1.55, { fill: C.navy });
  s.addText('"Lost, But Never Gone."', { x: 1.0, y: 5.05, w: 6.3, h: 0.55, fontFace: FONT_HEAD, fontSize: 22, italic: true, bold: true, color: C.gold });
  s.addText("The core brand promise driving every product decision, message and interaction.", { x: 1.0, y: 5.6, w: 6.3, h: 0.6, fontFace: FONT, fontSize: 12, color: C.textOnNavy });
  ...
  stats.forEach((st) => {
    ...
    s.addText(st[0], { x: 9.25, y: sy + 0.16, w: 3.2, h: 0.5, fontFace: FONT_HEAD, fontSize: 20, bold: true, color: C.textDark });
    s.addText(st[1], { x: 9.25, y: sy + 0.68, w: 3.2, h: 0.4, fontFace: FONT, fontSize: 11.5, color: C.textMuted });
    sy += 1.55;
  });
...
// ================= SLIDE 4 — RESEARCH METHODOLOGY =================
{
  ...
  s.addText("Our research began with lived, everyday experience rather than a desk study — then we validated it with peers before designing the product.", {
    x: 0.7, y: 1.62, w: 11.9, h: 0.5, fontFace: FONT, fontSize: 13, color: C.textMuted, italic: true,
  });
  ...
  steps.forEach((st, i) => {
    ...
    s.addText(st[1], { x: x + 0.24, y: my + 1.05, w: cw - 0.48, h: 0.65, fontFace: FONT, fontSize: 13, bold: true, color: C.textDark, lineSpacingMultiple: 1.05 });
    s.addText(st[2], { x: x + 0.24, y: my + 1.65, w: cw - 0.48, h: ch - 1.85, fontFace: FONT, fontSize: 10.2, color: C.textMuted, lineSpacingMultiple: 1.22 });
  });
...
// ================= SLIDE 5 — PROBLEM IDENTIFICATION =================
{
  ...
  s.addText("Losing keys, wallets, bags, or other everyday items is one of the most common and frustrating experiences people face — ranging from a minor inconvenience to a serious loss of IDs, passports, or medical items.", {
    x: 0.7, y: 1.72, w: 6.9, h: 1.05, fontFace: FONT, fontSize: 13, color: C.textDark, lineSpacingMultiple: 1.28,
  });
  ...
  pains.forEach((p) => {
    ...
    s.addText(p[1], { x: 1.38, y: py - 0.03, w: 6.25, h: 0.55, fontFace: FONT, fontSize: 11.8, color: C.textDark, lineSpacingMultiple: 1.15, valign: "middle" });
    py += 0.72;
  });

  // right: informal survey snapshot
  card(s, 8.0, 1.72, 4.65, 5.05, { fill: C.navy });
  s.addText("PRIMARY RESEARCH SNAPSHOT", { x: 8.35, y: 1.95, w: 4, h: 0.32, fontFace: FONT, fontSize: 11, bold: true, color: C.lavender, charSpacing: 1.6 });
  s.addText("Informal survey of ~40 students & hostel residents", { x: 8.35, y: 2.28, w: 4, h: 0.35, fontFace: FONT, fontSize: 10, italic: true, color: C.textOnNavyMuted });
  ...
  surveyRows.forEach((r) => {
    s.addText(r[1], { x: 8.35, y: ry, w: 1.15, h: 0.55, fontFace: FONT_HEAD, fontSize: 24, bold: true, color: C.gold, valign: "middle" });
    s.addText(r[0], { x: 9.6, y: ry, w: 2.9, h: 0.55, fontFace: FONT, fontSize: 10.8, color: C.white, valign: "middle", lineSpacingMultiple: 1.1 });
    ry += 0.78;
  });
  s.addShape("line", { x: 8.35, y: ry + 0.02, w: 3.95, h: 0, line: { color: "3B3164", width: 1 } });
  s.addText("Directional insight from our own research process — not a published statistic.", { x: 8.35, y: ry + 0.15, w: 4, h: 0.5, fontFace: FONT, fontSize: 9, italic: true, color: C.textOnNavyMuted, lineSpacingMultiple: 1.15 });
...
// ================= SLIDE 6 — MARKET OPPORTUNITY =================
{
  ...
  s.addText("Industry market-research reports place the global smart tracker category in the hundreds of millions to low billions of dollars today, with most forecasts pointing to sustained double-digit annual growth (CAGR) through the early 2030s — but coverage of affordable, privacy-first solutions for developing markets remains thin.", {
    x: 0.7, y: 1.68, w: 11.9, h: 0.85, fontFace: FONT, fontSize: 12.3, color: C.textMuted, lineSpacingMultiple: 1.25,
  });

  // chart: simple growth trend
  const chartData = [{
  ...
  facts.forEach((f) => {
    ...
    s.addText(f[1], { x: 8.1, y: fy + 0.12, w: 4.4, h: 0.35, fontFace: FONT, fontSize: 13, bold: true, color: C.textDark });
    s.addText(f[2], { x: 8.1, y: fy + 0.47, w: 4.4, h: 0.65, fontFace: FONT, fontSize: 10, color: C.textMuted, lineSpacingMultiple: 1.15 });
    fy += 1.36;
  });
...
// ================= SLIDE 7 — PROPOSED SOLUTION =================
{
  ...
  s.addText("Not just a keychain — a complete privacy-first ecosystem. All products connect to one platform, share the same secure contact system, and deliver the same instant notification experience.", {
    x: 0.7, y: 1.65, w: 11.9, h: 0.6, fontFace: FONT, fontSize: 12.5, color: C.textMuted, lineSpacingMultiple: 1.25,
  });
  ...
  const fx = 0.7, fy = 2.55, fw = 3.55, gap = 0.95, fh = 1.9;
  flow.forEach((f, i) => {
    const x = fx + i * (fw + gap);
    card(s, x, fy, fw, fh, {});
    iconChip(s, f[0], "white", x + 0.25, fy + 0.25, 0.8, C.purple);
    s.addText(f[1], { x: x + 0.25, y: fy + 1.1, w: fw - 0.5, h: 0.35, fontFace: FONT, fontSize: 14.5, bold: true, color: C.textDark });
    s.addText(f[2], { x: x + 0.25, y: fy + 1.42, w: fw - 0.5, h: 0.45, fontFace: FONT, fontSize: 9.8, color: C.textMuted, lineSpacingMultiple: 1.12 });
    if (i < flow.length - 1) {
  ...
  s.addText("Privacy-First Design — Vouch's Strongest Differentiator", { x: 2.15, y: 4.98, w: 9.9, h: 0.4, fontFace: FONT_HEAD, fontSize: 15.5, bold: true, color: C.white });
  s.addText("When a finder scans a Vouch QR, they see a secure contact page — never a real name, phone number, or address. The owner controls exactly what's shared: first name only, masked-number calling, or emergency contact info for critical items.", {
    x: 2.15, y: 5.4, w: 9.9, h: 0.85, fontFace: FONT, fontSize: 11.3, color: C.textOnNavy, lineSpacingMultiple: 1.25,
  });
...
// ================= SLIDE 8 — PRODUCT LINE =================
{
  ...
  products.forEach((p, i) => {
    ...
    s.addText(p[1], { x: x + 0.15, y: my + 1.3, w: cw - 0.3, h: 0.55, align: "center", fontFace: FONT_HEAD, fontSize: 15, bold: true, color: isFlag ? C.white : C.textDark });
    s.addText(p[2], { x: x + 0.15, y: my + 1.82, w: cw - 0.3, h: 0.4, align: "center", fontFace: FONT, fontSize: 14, bold: true, color: isFlag ? C.gold : C.purpleDeep });
    s.addShape("line", { x: x + 0.3, y: my + 2.32, w: cw - 0.6, h: 0, line: { color: isFlag ? "3B3164" : C.line, width: 1 } });
    s.addText(p[3], { x: x + 0.22, y: my + 2.5, w: cw - 0.44, h: 1.9, align: "center", fontFace: FONT, fontSize: 10, color: isFlag ? C.textOnNavyMuted : C.textMuted, lineSpacingMultiple: 1.22 });
    if (isFlag) {
  ...
  s.addText("All four connect to the same app & website: multi-item dashboard, Lost/Safe toggle, instant alerts, scan log, and secure finder messaging.", {
    x: 0.7, y: my + ch + 0.18, w: 11.9, h: 0.4, fontFace: FONT, fontSize: 10.5, italic: true, color: C.textMuted,
  });
...
// ================= SLIDE 9 — TARGET MARKET & PERSONAS =================
{
  ...
  const mx = 0.7, my = 1.75, gap = 0.2, cw = (PGW - mx * 2 - gap * 4) / 5, ch = 1.95;
  segs.forEach((sg, i) => {
    const x = mx + i * (cw + gap);
    card(s, x, my, cw, ch, {});
    iconChip(s, sg[0], "white", x + cw / 2 - 0.34, my + 0.2, 0.68, C.purple);
    s.addText(sg[1], { x: x + 0.1, y: my + 1.0, w: cw - 0.2, h: 0.42, align: "center", fontFace: FONT, fontSize: 11.3, bold: true, color: C.textDark, lineSpacingMultiple: 1.05 });
    s.addText(sg[2], { x: x + 0.12, y: my + 1.4, w: cw - 0.24, h: 0.55, align: "center", fontFace: FONT, fontSize: 8.6, color: C.textMuted, lineSpacingMultiple: 1.15 });
  });

  // persona spotlight card
  const py = my + ch + 0.3;
  card(s, 0.7, py, 11.93, 2.3, { fill: C.navy });
  iconChip(s, "grad", "navy", 1.0, py + 0.35, 1.55, C.lavender, 0.5);
  s.addText("PERSONA SPOTLIGHT", { x: 2.85, y: py + 0.22, w: 5, h: 0.3, fontFace: FONT, fontSize: 10.5, bold: true, color: C.lavender, charSpacing: 1.8 });
  s.addText("\u201CAyesha, 20 — University Student\u201D", { x: 2.85, y: py + 0.5, w: 8.7, h: 0.4, fontFace: FONT_HEAD, fontSize: 17, bold: true, color: C.white });
  s.addText("Lives in a university hostel; loses her room keys and student ID at least once a month, and once lost her wallet on campus for two days. Owns a mid-range smartphone, is budget-conscious, and is highly active on Instagram/TikTok.", {
    x: 2.85, y: py + 0.95, w: 8.7, h: 0.75, fontFace: FONT, fontSize: 10.8, color: C.textOnNavy, lineSpacingMultiple: 1.25,
  });
  s.addText("Goal: Never worry about a lost hostel key again — for under the price of a canteen meal.", {
    x: 2.85, y: py + 1.68, w: 8.7, h: 0.4, fontFace: FONT, fontSize: 10.5, italic: true, color: C.gold,
  });
...
// ================= SLIDE 10 — SWOT ANALYSIS =================
{
  ...
  quad.forEach((q, i) => {
    ...
    s.addText(q.label, { x: x + 0.85, y: y + 0.2, w: cw - 1, h: 0.5, fontFace: FONT_HEAD, fontSize: 15.5, bold: true, color: C.textDark, valign: "middle" });
    ...
    s.addText(bullets, { x: x + 0.28, y: y + 0.78, w: cw - 0.5, h: ch - 0.95, fontFace: FONT, fontSize: 9.6, lineSpacingMultiple: 1.18 });
  });
...
// ================= SLIDE 11 — MARKETING STRATEGY (STP) =================
{
  ...
  stp.forEach((col, i) => {
    ...
    s.addText(col[1], { x: x + 1.1, y: my + 0.34, w: cw - 1.3, h: 0.55, fontFace: FONT_HEAD, fontSize: 16.5, bold: true, color: C.textDark, valign: "middle" });
    ...
    s.addText(bullets, { x: x + 0.26, y: my + 1.35, w: cw - 0.52, h: ch - 1.55, fontFace: FONT, fontSize: 10.3, lineSpacingMultiple: 1.2 });
  });
...
// ================= SLIDE 12 — VALUE PROPOSITION & POSITIONING MAP =================
{
  ...
  s.addText("VALUE PROPOSITION", { x: 1.0, y: 1.98, w: 5, h: 0.3, fontFace: FONT, fontSize: 10.5, bold: true, color: C.lavender, charSpacing: 1.8 });
  s.addText("\u201CProtect everything you carry, without exposing who you are \u2014 at a price everyone can afford.\u201D", {
    x: 1.0, y: 2.28, w: 4.95, h: 1.0, fontFace: FONT_HEAD, fontSize: 13.5, italic: true, bold: true, color: C.white, lineSpacingMultiple: 1.2,
  });
  ...
  props.forEach((p) => {
    ...
    s.addText(p[1], { x: 1.35, y: vy - 0.03, w: 4.9, h: 0.32, fontFace: FONT, fontSize: 11.8, bold: true, color: C.textDark });
    s.addText(p[2], { x: 1.35, y: vy + 0.28, w: 4.9, h: 0.4, fontFace: FONT, fontSize: 9.8, color: C.textMuted, lineSpacingMultiple: 1.15 });
    vy += 0.78;
  });

  // Positioning map: Price (y, low top->high bottom) vs Privacy & Tech (x)
  const mapX = 6.75, mapY = 1.8, mapW = 5.9, mapH = 5.3;
  card(s, mapX, mapY, mapW, mapH, {});
  s.addText("POSITIONING MAP", { x: mapX + 0.3, y: mapY + 0.22, w: 4, h: 0.3, fontFace: FONT, fontSize: 10.5, bold: true, color: C.purple, charSpacing: 1.6 });
  s.addText("Price vs. Privacy & Smart-Tech Features", { x: mapX + 0.3, y: mapY + 0.52, w: 5, h: 0.3, fontFace: FONT, fontSize: 10, italic: true, color: C.textMuted });
  ...
  s.addText("HIGH PRICE", { x: plotX - 0.65, y: plotY - 0.28, w: 1.3, h: 0.24, align: "center", fontFace: FONT, fontSize: 7.5, color: C.textMuted, charSpacing: 0.5 });
  s.addText("LOW PRICE", { x: plotX - 0.65, y: plotY + plotH + 0.05, w: 1.3, h: 0.24, align: "center", fontFace: FONT, fontSize: 7.5, color: C.textMuted, charSpacing: 0.5 });
  s.addText("LOW PRIVACY / TECH", { x: plotX - 0.5, y: plotY + plotH + 0.32, w: 1.9, h: 0.24, fontFace: FONT, fontSize: 7.5, color: C.textMuted, charSpacing: 0.3 });
  s.addText("HIGH PRIVACY / TECH", { x: plotX + plotW - 1.5, y: plotY + plotH + 0.32, w: 2.0, h: 0.24, align: "right", fontFace: FONT, fontSize: 7.5, color: C.textMuted, charSpacing: 0.3 });
  ...
  points.forEach((p) => {
    ...
    s.addText(label, { x: px - 0.6 + d / 2, y: py2 + d + 0.03, w: 1.5, h: 0.24, align: "center", fontFace: FONT, fontSize: isVouch ? 9.5 : 8, bold: isVouch, color: isVouch ? C.purpleDeep : C.textMuted });
  });
...
// ================= SLIDE 13 — BRAND ELEMENTS: LOGO & SLOGAN =================
{
  ...
  s.addText('"Lost, But Never Gone."', { x: 0.9, y: 5.55, w: 4.45, h: 0.4, align: "center", fontFace: FONT_HEAD, fontSize: 15, italic: true, bold: true, color: C.gold });
  s.addText("Primary Slogan", { x: 0.9, y: 5.95, w: 4.45, h: 0.3, align: "center", fontFace: FONT, fontSize: 10, color: C.textOnNavyMuted, charSpacing: 1.2 });
  s.addShape("line", { x: 1.3, y: 6.3, w: 3.65, h: 0, line: { color: "3B3164", width: 1 } });
  s.addText('"Connect. Protect. Locate."', { x: 0.9, y: 6.4, w: 4.45, h: 0.35, align: "center", fontFace: FONT, fontSize: 12.5, italic: true, color: C.lavender });
  s.addText("Secondary Tagline", { x: 0.9, y: 6.72, w: 4.45, h: 0.28, align: "center", fontFace: FONT, fontSize: 9, color: C.textOnNavyMuted, charSpacing: 1.2 });
  ...
  brandBits.forEach((b) => {
    ...
    s.addText(b[1], { x: 6.65, y: by - 0.02, w: 6.0, h: 0.35, fontFace: FONT, fontSize: 13.5, bold: true, color: C.white });
    s.addText(b[2], { x: 6.65, y: by + 0.35, w: 6.0, h: 0.85, fontFace: FONT, fontSize: 10.8, color: C.textOnNavyMuted, lineSpacingMultiple: 1.25 });
    by += 1.15;
  });

  card(s, 5.85, by + 0.1, 6.78, 1.35, { fill: C.navy2, lineColor: "3B3164" });
  s.addText("Brand promise in one line:", { x: 6.15, y: by + 0.28, w: 6.2, h: 0.3, fontFace: FONT, fontSize: 10, color: C.textOnNavyMuted });
  s.addText("Whatever you lose, Vouch keeps the door open to getting it back \u2014 safely and privately.", {
    x: 6.15, y: by + 0.58, w: 6.2, h: 0.75, fontFace: FONT_HEAD, fontSize: 13, italic: true, color: C.lavender, lineSpacingMultiple: 1.2,
  });
...
// ================= SLIDE 14 — PRICING STRATEGY =================
{
  ...
  s.addText("Hardware is priced by tier to match the value delivered and local purchasing power; the platform layers a freemium subscription on top for recurring revenue.", {
    x: 0.7, y: 1.65, w: 11.9, h: 0.55, fontFace: FONT, fontSize: 12.3, color: C.textMuted, lineSpacingMultiple: 1.25,
  });
  ...
  tiers.forEach((t, i) => {
    ...
    s.addText(t[1], { x: x + 0.24, y: my + 0.94, w: cw - 0.48, h: 0.32, fontFace: FONT, fontSize: 12, bold: true, color: isFlag ? C.white : C.textDark });
    s.addText(t[2], { x: x + 0.24, y: my + 1.24, w: cw - 0.48, h: 0.35, fontFace: FONT_HEAD, fontSize: 15, bold: true, color: isFlag ? C.gold : C.purpleDeep });
    s.addText(t[3], { x: x + 0.24, y: my + 1.58, w: cw - 0.48, h: 0.3, fontFace: FONT, fontSize: 8.8, italic: true, color: isFlag ? C.textOnNavyMuted : C.textMuted });
  });
  ...
  s.addText("Platform: Freemium Subscription", { x: 1.7, y: py + 0.2, w: 4.6, h: 0.35, fontFace: FONT, fontSize: 13, bold: true, color: C.textDark });
  s.addText("Hardware is a one-time purchase. The free tier covers core scan-and-notify functionality for every buyer; a premium subscription unlocks GPS live tracking history, multi-item family plans, and priority alerts.", {
    x: 0.98, y: py + 0.75, w: 5.2, h: 1.3, fontFace: FONT, fontSize: 10.3, color: C.textDark, lineSpacingMultiple: 1.25,
  });

  card(s, 6.7, py, 5.93, 2.2, { fill: C.navy });
  s.addText("WHY THIS PRICING WORKS", { x: 6.98, y: py + 0.2, w: 5.4, h: 0.3, fontFace: FONT, fontSize: 10.5, bold: true, color: C.lavender, charSpacing: 1.6 });
  const rationale = [
  ...
  s.addText(rationale, { x: 6.98, y: py + 0.58, w: 5.4, h: 1.5, fontFace: FONT, fontSize: 10.3, lineSpacingMultiple: 1.25 });

  footer(s, 14, false);
}

// ---------- WRITE FILE ----------
pres.writeFile({ fileName: "Vouch_Presentation_First_14.pptx" }).then(() => {
  console.log("Successfully wrote Vouch_Presentation_First_14.pptx");
}).catch(err => {
