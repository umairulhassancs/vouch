...
// ── SVG Icon paths (Feather/Lucide style, 24x24 viewBox) ──
const IC = {
  ...
  qrcode: '<rect x="2" y="2" width="8" height="8" rx="1"/><rect x="14" y="2" width="8" height="8" rx="1"/><rect x="2" y="14" width="8" height="8" rx="1"/><rect x="14" y="14" width="4" height="4" rx="0.5"/><line x1="22" y1="14" x2="22" y2="22"/><line x1="14" y1="22" x2="22" y2="22"/>',
  ...
  store: '<path d="M3 9l2-7h14l2 7"/><path d="M3 9v11a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V9"/><path d="M9 22V12h6v10"/><path d="M3 9c0 1.5 1 3 2.5 3S8 10.5 8 9m0 0c0 1.5 1.2 3 2.5 3s2.5-1.5 2.5-3m0 0c0 1.5 1 3 2.5 3s2.5-1.5 2.5-3"/>',
  rocket: '<path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.13-.09-2.91a2.18 2.18 0 0 0-2.91-.09z"/><path d="m12 15-3-3a22 22 0 0 1 2-3.95A12.88 12.88 0 0 1 22 2c0 2.72-.78 7.5-6 11a22.35 22.35 0 0 1-4 2z"/><path d="M9 12H4s.55-3.03 2-4c1.62-1.08 5 0 5 0"/><path d="M12 15v5s3.03-.55 4-2c1.08-1.62 0-5 0-5"/>',
  check: '<polyline points="20 6 9 17 4 12"/>',
};

function icon(name, color = 'currentColor') {
  return `<svg class="ic" viewBox="0 0 24 24" fill="none" stroke="${color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${IC[name] || ''}</svg>`;
}

function iconChip(name, bg, color = '#fff', size = 'md') {
  return `<div class="icon-chip icon-chip--${size}" style="background:${bg}">${icon(name, color)}</div>`;
}

// ── SLIDE CONTENT DATA ──
const slides = [...require('./slide_data_1.js'), ...require('./slide_data_2.js')];

// ── BUILD HTML ──
const CSS = fs.readFileSync(path.join(__dirname, 'html_styles.css'), 'utf-8');
const JS = fs.readFileSync(path.join(__dirname, 'html_script.js'), 'utf-8');
const SLIDES_HTML = slides.map((s, i) => s(i + 1, { icon, iconChip, LOGO })).join('\n');
...
<title>Vouch — Principles of Marketing Presentation</title>
...
<div id="app">
  ...
  <div id="progress"></div>
  <div id="counter"><span id="cur">01</span> / <span id="tot">20</span></div>
  <button id="prev" class="nav-btn" aria-label="Previous">&#8592;</button>
  <button id="next" class="nav-btn" aria-label="Next">&#8594;</button>
  <button id="fs-btn" class="nav-btn nav-btn--fs" aria-label="Fullscreen" title="Press F for fullscreen">&#x26F6;</button>
</div>
<script>${JS}</script>
</body>
</html>`;

const outDir = path.join('C:\\Users\\Umair Ul Hassan\\Downloads\\vouch');
if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
fs.writeFileSync(path.join(outDir, 'presentation.html'), html);
console.log('✅ Successfully wrote presentation.html');
